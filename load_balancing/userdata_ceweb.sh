...
exec > >(tee /var/log/userdata_ceweb.log) 2>&1
echo "Creative Energy Web Server preparation started: $(date)"
...
echo "Creating master_config.json for Creative Energy Web Server..."
cat > /home/rocky/master_config.json << 'MASTER_CONFIG_EOF'
{
  "config_metadata": {
    "version": "1.0.0",
    "created": "$(date -I)",
    "description": "Creative Energy Web Server Configuration for Cross VPC Load Balancing",
    "server_role": "ceweb_server",
...
if [ -f /home/rocky/ceweb/web-server/install_web_server.sh ]; then
    ...
    chmod +x /home/rocky/ceweb/web-server/install_web_server.sh
else
...
ping -c 2 10.1.1.110 && echo "✓ Bastion (10.1.1.110) connectivity OK" || echo "❌ Bastion connectivity failed"
ping -c 2 10.2.1.211 && echo "✓ BB Web (10.2.1.211) connectivity OK" || echo "❌ BB Web connectivity failed (VPC Peering needed)"
...
Creative Energy Web Server preparation completed: $(date)

=== Ready for Manual Installation ===

Server Role: Creative Energy Web Server (VPC1)
Server IP: 10.1.1.111
Peer Server: Big Boys Web Server (10.2.1.211)

Next steps:
1. SSH to this server (CE Web): ssh -i your-key.pem rocky@10.1.1.111
2. Run installation: cd /home/rocky/ceweb/web-server && sudo bash install_web_server.sh
...
Log file: /var/log/userdata_ceweb.log
...
echo "Creative Energy Web Server preparation completed: $(date)"
